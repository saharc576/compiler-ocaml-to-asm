...
%macro SHIFT_FRAME 1 ; %1 = loop counter = 2 + new_n + 3
	push rax
	push qword [rbp]			; pushing old rbp
	mov rax, PARAM_COUNT
	; include the (ret, env, n) of the old frame
	add rax, 3					; rax has the old frame size 
								; this is the num of cells to move up
	shl rax, 3					; in bytes

	mov r15, %1		
	mov rcx, 1	
	lea rbx, [rbp + rax]	; the first address to move to
%%start_loop:
	cmp rcx, r15
	je %%end_loop

	lea r9, [rbp]			
	shl rcx, 3				; rcx in bytes
	sub r9, rcx 			; the address to move from
	shr rcx, 3				; rcx as cells number
	mov r9, [r9]			; the value to move
	mov [rbx], r9
	inc rcx
	sub rbx, WORD_SIZE		; point the next (=below) cell
	jmp %%start_loop
%%end_loop:
	pop r10						; poping the old rbp address
	pop rax						; restoring rax
	lea rsp, [rbx + WORD_SIZE]	; rbx points to the next empty cell
	mov rbp, r10				; return the previous rbp					

%endmacro

%macro INC_FRAME 0
	mov r13, rax				; store rax 
	mov rax, qword [rsp + 8*2]
	add rax, 3					; include the (ret, env, n)				

	lea rbx, [rsp]	
	sub rbx, WORD_SIZE			; the first address to move to = one cell bellow
%%start_loop:
	cmp rax, 0					; check if moved all
	je %%end_loop				; finish if yes
	; else load the next address to move elemnt from
	lea r9, [rbx + WORD_SIZE]	; the current cell to move from (address)
	mov r10, r9					; store for later use (=add_nil)
	mov r9, [r9]				; the value to move
	mov [rbx], r9				; move to rbx 
	add rbx, WORD_SIZE			; point the next (=above) cell
	dec rax						; update loop counter
	jmp %%start_loop
%%end_loop:
	mov rax, r13
	sub rsp, WORD_SIZE
%%inc_argc:
	mov r8, [rsp+2*WORD_SIZE]
	inc r8
